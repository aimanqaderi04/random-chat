...
if (chatHistory) {
    ...
    async data => {
        ...
        const reporterId =
            getPermanentId(socket.id);

        const reportedId =
            getPermanentId(partnerId);

        if (!reporterId || !reportedId) {
            return;
        }

        let reason = "Nicht angegeben";
        let description = "";
        ...
        const prompt = `
Du bist eine Moderations-KI für einen anonymen 1-zu-1-Chat.

Prüfe eine Benutzer-Meldung anhand des Meldegrundes
und des gesamten Chatverlaufs.
...
Chatverlauf:
${chatText || "Kein Chatverlauf vorhanden"}

WICHTIG:
- Beurteile den Kontext des gesamten Chats.
- Ein einzelnes Schimpfwort reicht nicht automatisch für einen Bann.
- Entscheide nur, wenn ein tatsächlicher Regelverstoß erkennbar ist.
- Eine normale Diskussion ist kein Regelverstoß.
...
`;
        ...
        try {
            ...
            console.error(
                "❌ KI konnte nicht erreicht werden:",
                error.message
            );

            // Wenn die KI nicht erreichbar ist:
            // KEIN automatischer Bann.
            socket.emit(
                "report received",
                {
                    success: false,
                    message:
                        "Die Meldung konnte gerade nicht geprüft werden."
                }
            );
        ...
        if (!reportAccepted) {
            ...
            socket.emit(
                "report received",
                {
                    success: false,
                    message:
                        "Die Meldung wurde nach Prüfung nicht bestätigt."
                }
            );
        ...
        socket.emit(
            "report received",
            {
                success: true,
                message:
                    "Die Meldung wurde von der KI bestätigt."
            }
        );
